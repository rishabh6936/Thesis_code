...
class GeometricDataObject:
        ...
        def process_graph(self):
            ...
            self.graph.substring_node_dict = self.get_node_type_indexes('substring')


            self.graph.ei_mentions = self.get_edge_type_indexes('sentence', 'noun', 'mentions', node_dict)
            self.graph.ei_mentions = self.map_to_local_indices(self.graph.ei_mentions)
            ...
            self.graph.ei_child_of = self.get_edge_type_indexes('email', 'sentence', 'child_of', node_dict)
            self.graph.ei_child_of = self.map_to_local_indices(self.graph.ei_child_of)

            self.graph.ei_father_of = self.get_edge_type_indexes('sentence', 'email', 'father_of', node_dict)
            self.graph.ei_father_of = self.map_to_local_indices(self.graph.ei_father_of)


            self.graph.ei_comes_after_sent = self.get_edge_type_indexes('sentence', 'sentence', 'comes_after', node_dict)
            self.graph.ei_comes_after_sent = self.map_to_local_indices(self.graph.ei_comes_after_sent)
            ...
            self.graph.ei_comes_before = self.get_edge_type_indexes('substring', 'noun', 'comes_before', node_dict)
            self.graph.ei_comes_before = self.map_to_local_indices(self.graph.ei_comes_before)

            self.graph.ei_comes_before_noun = self.get_edge_type_indexes('noun', 'substring', 'comes_before', node_dict)
            self.graph.ei_comes_before_noun = self.map_to_local_indices(self.graph.ei_comes_before_noun)

            self.graph.ei_comes_after = self.get_edge_type_indexes('substring', 'noun', 'comes_after', node_dict)
            self.graph.ei_comes_after = self.map_to_local_indices(self.graph.ei_comes_after)

            self.graph.ei_comes_after_noun = self.get_edge_type_indexes('noun', 'substring', 'comes_after', node_dict)
            self.graph.ei_comes_after_noun = self.map_to_local_indices(self.graph.ei_comes_after_noun)
        ...
        def make_hetero_object(self):
            ...
            self.hetero_data['substring'].x = self.get_node_feature_matrix(self.graph.substring_node_dict)


            # Set the edge indices for email to noun relationships
            self.hetero_data['sentence', 'mentions', 'noun'].edge_index = self.graph.ei_mentions
            ...
            self.hetero_data['email', 'child_of', 'sentence'].edge_index = self.graph.ei_child_of

            self.hetero_data['sentence', 'father_of', 'email'].edge_index = self.graph.ei_father_of
            ...
            self.hetero_data['substring', 'comes_before', 'noun'].edge_index = self.graph.ei_comes_before
            self.hetero_data['noun', 'comes_before', 'substring'].edge_index = self.graph.ei_comes_before_noun


            self.hetero_data['substring', 'comes_after', 'noun'].edge_index = self.graph.ei_comes_after
            self.hetero_data['noun', 'comes_after', 'substring'].edge_index = self.graph.ei_comes_after_noun

            self.hetero_data['sentence', 'comes_after', 'sentence'].edge_index = self.graph.ei_comes_after_sent


            """for edge_type in self.hetero_data.edge_types:
                # Only assign is_predicted for ('email', 'belongs_to', 'noun') edge type
                if edge_type == ('email', 'belongs_to', 'noun'):
                    self.hetero_data[edge_type].is_predicted = torch.tensor(self.is_predicted, dtype=torch.bool)"""
